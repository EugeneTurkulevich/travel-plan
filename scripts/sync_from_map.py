...
Що робить (докладніше — у коментарях коду):
  ...
  (в) наявні картки НІКОЛИ не переписуються. Якщо картка стверджує роль, яка
      розійшлась зі станом карти (лишилась «альтернативою», хоч тепер у
      маршруті) — це йде окремим блоком звіту, а не мовчки виправляється.
...
# геокодування; менші дистанції (Сібіу — 0.008°) впевнено проходять.
...
def strip_html(raw):
    """popup_html → чиста проза: блокові теги стають переносами рядка, решта
    тегів (у т.ч. <img>) прибирається без сліду, HTML-сутності розкодовано.
    Не намагається відновити структуру абзаців джерела — це «базовий»
    рівень, позначений як не вичитаний."""
    if not raw:
        return ""
...
def build_new_card_md(slug, title, lat, lon, code, country_name, flag, popup_html,
                       nested_points, import_date_ukr):
    desc = strip_html(popup_html)
    if not desc:
    ...
    poi_table = render_poi_table(nested_points)
    if poi_table:
        lines += ["", "---", "", "## Цікаві POI", "", poi_table]
...
def resolve_point_slug(mp, old_by_mpid, library_index, taken_slugs, report, new_cards_pending):
    """Slug для точки карти mp. Порядок: map_point_id у старому route.json →
    пошук у бібліотеці за координатами/назвою → нова картка (§ б)."""
    mpid = mp["id"]
    ...
    base_slug = slugify(mp.get("label") or mpid, code if code != "?" else "xx")
...
def build_new_days(mirror_points, old_route_json, old_by_mpid, library_index, taken_slugs,
    ...
    for mp in mirror_points:
        ...
        slug, old_point, origin = resolve_point_slug(
            mp, old_by_mpid, library_index, taken_slugs, report, new_cards_pending
        )
...
def process_alternates(alt_points, old_alternates, library_index, taken_slugs, report,
                        new_cards_pending, slugmap_may_count, import_date_ukr):
    ...
    for ap in alt_points:
        ...
        base_slug = slugify(name, code if code != "?" else "xx")
        slug = unique_slug(base_slug, taken_slugs)
        taken_slugs.add(slug)
        new_cards_pending.append({
