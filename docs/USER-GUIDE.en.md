...
- **🛏 Overnight place** / **🅿️ Parking place** — right-click on the
  ...
  … here"** button in the pin's popup (the ellipsis is the point's name)
  does the same thing manually at any time — handy if the main point ever got dragged somewhere else
...
it's simply not in the menu. The layer shows on the map every event
with coordinates you log in a separate app of yours (MyMyCar): fuel,
parking, tolls, odometer readings — and everything else (service, other
expenses, etc.) as one shared marker — without importing that data
...
colored by type (⛽️ fuel, 🅿️ parking, 🛣️ toll, 🔢 odometer, 🚗 any other
event — one purple marker for all of them); zoom in further (and
